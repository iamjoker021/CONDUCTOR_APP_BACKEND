...
async function intializeData(db) {
    let isDataExists;
    isDataExists = await new Promise((resolve, reject) => {
  ...
  db.exec(`
    ...
    route_id, city_id, source, destination, 
    avg_travel_time
    ) 
    VALUES 
    (
        1, 1, 'Times Square', 'Central Park', 
        30
    ), 
    (
        2, 1, 'Wall Street', 'Brooklyn Bridge', 
        25
    ), 
    (
        3, 2, 'Golden Gate', 'Fishermans Wharf', 
        20
    ), 
    (
        4, 2, 'Union Square', 'Alcatraz Island', 
        40
    ), 
    (
        5, 3, 'Navy Pier', 'Millennium Park', 
        15
    ), 
    (
        6, 3, 'Willis Tower', 'Magnificent Mile', 
        10
    );
    INSERT INTO busstops (
    stop_id, route_id, stop_name, stop_order, 
    distance_from_start
    ) 
    VALUES 
    ...
    INSERT INTO buses (
    bus_id, route_id, bus_number, bus_type, 
    price
    ) 
    VALUES 
    (1, 1, 'NY001', 'AC', 3.00), 
    ...
    after 
    INSERT ON tickets FOR EACH row when (
        new.ticket_unique_identifier IS NULL
    ) begin 
    UPDATE 
    tickets 
    SET 
    ...
    after 
    INSERT ON users FOR EACH row when (new.id IS NULL) begin 
    UPDATE 
    users 
    SET 
    id = (
